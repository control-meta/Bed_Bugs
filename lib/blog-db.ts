...
export function readLocalBlogs(): BlogItem[] {
  try {
    ...
    return parsed.map((item: any) => ({
      ...
      imageUrl: item.imageUrl || item.image_url || (item.markdown ? (item.markdown.match(/!\[.*?\]\((.*?)\)/)?.[1] || "") : "") || "/images/blogs/bed-bugs-pest-control.png",
...
function rowToBlog(row: any): BlogItem {
  return {
    ...
    excerpt: row.excerpt || "",
    imageUrl: row.image_url || (row.markdown ? (row.markdown.match(/!\[.*?\]\((.*?)\)/)?.[1] || "") : "") || "/images/blogs/bed-bugs-pest-control.png",
